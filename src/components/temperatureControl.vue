  ...
  methods:{
    ...
    val (nv) {
      if (null != nv) {
        ...
        b = a = Math.atan2(this.mx - this.x, -( this.my -  this.y -  this.opt.width / 2)) - this.opt.angleOffset;
        (a < 0) && (b = a + this.PI2);
        //四舍五入
        nv = Math.round(b * (this.opt.max - this.opt.min) / this.PI2) + Number(this.opt.min);
        this.setVal(nv);
...
//        ,tick
      ...
      switch (this.opt.skin) {
        case 'default' :
          ...
          this.ctx.arc(r *  Math.cos(eat-0.0)+o,r * Math.sin(eat-0.0)+ o,r/10,this.PI2,0,true);
    ...
    startDrag(e) {
      var p = {top : this.ctx.canvas.offsetTop,left : this.ctx.canvas.offsetLeft}
//      var p = this.ctx.offset()
        ,$doc = this.ctx;
      //计算圆点
